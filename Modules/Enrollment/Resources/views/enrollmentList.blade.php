...
<div class="container-fluid">
    <div class="fade-in">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header"><i class="fa fa-align-justify"></i> Enrollment List</div>
                <div class="card-body">
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            @forelse ($enrollments as $key=>$contactus)
                            <tr>
                                <td>{{ $key+1 }}</td>
                                <td>{{ $contactus->name }}</td>
                                <td>{{ $contactus->email }}</td>
                                <td>{{ $contactus->mobile_phone_no }}</td>
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("enrollment.delete", ":id") }}';
