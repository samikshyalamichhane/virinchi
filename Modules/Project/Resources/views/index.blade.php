...
<button class="float-right p-2 mb-2 btn btn-sm btn-primary" style="margin-top:10px"
    onclick="window.location='{{ route('projects.create') }}'">Add
    Project</button>
@endcan
@endsection
@section('content')
                    ...
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $project->title }}</td>
                                <td>
                                    <img src="{{Storage::url($project->image)}}" height="50">
                                </td>
                                ...
                                <td>
                                    <button class="btn btn-xs btn-primary"
                                        onclick="window.location=`{{ route('projectgallery.create',['id'=>$project->id]) }}`">Add Gallery</button>
                                    <button class="btn btn-xs btn-info"
                                    onclick="window.location=`{{ route('projects.edit',['id'=>$project->id]) }}`">Edit</button>
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("projects.delete", ":id") }}';
