...
<button class="float-right p-2 mb-2 btn btn-sm btn-primary" style="margin-top:10px"
    onclick="window.location='{{ route('partner.create') }}'">Add
    Partner</button>
@endcan
@endsection
@section('content')
<div class="container-fluid">
    <div class="fade-in">
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header"><i class="fa fa-align-justify"></i> Partner</div>
                <div class="card-body">
                    <table class="table table-responsive-sm table-bordered table-striped table-sm">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $partner->title }}</td>
                                <td>
                                    {{ Str::limit(html_entity_decode(strip_tags($partner->description)),200,'......') }}
                                </td>
                                <td>{{ $partner->country }}</td>
                                <td>{{ $partner->city }}</td>
                                <td>
                                    <img src="{{Storage::url($partner->image)}}" height="100">
                                </td>
                                <td>
                                    {!! $partner->publish?'<span
                                        class="badge badge-pill badge-success">Active</span>':'<span
                                        class="badge badge-pill badge-warning">Inactive</span>' !!}
                                </td>
                                <td>
                                    <button class="btn btn-xs btn-info"
                                    onclick="window.location=`{{ route('partner.edit',['id'=>$partner->id]) }}`">Edit</button>
                        ...
                        <div class="team">
                            <div class="row">
                                <div class="name col-sm-6">
                                    <p> <strong>Partner Title</strong> :
                                        {{ $partner->title }}
                                    </p>
                                </div>
                                <div class="name col-sm-6">
                                    <p> <strong>Partner Link</strong> :
                                        {{ $partner->partner_link }}
                                    </p>
                                </div>
                                <div class="name col-sm-12">
                                    <p> <strong>Partner Short Description</strong> :
                                        {{ Str::limit(html_entity_decode(strip_tags($partner->description)),200,'......') }}
                                    </p>
                                </div>
                                <div class="team-title col-sm-12">
                                    <img src="{{Storage::url($partner->image)}}" height="200">
                                </div>
                            </div>
                            <div class="details">
                                <div class="staus">
                                ...
                                <div class="action d-flex">
                                    <div class="p-3 edit"
                                        onclick="window.location=`{{ route('partner.edit',['id'=>$partner->id]) }}`">
                                        <i class="fas fa-edit"></i>
                                    </div>
                                    <div class="p-3 delete" data-toggle="confirm" data-id="{{ $partner->id }}">
...
<script>
    $(function(){
        $('[data-toggle="confirm"]').jConfirm().on('confirm', function(e){
            var btn = $(this),
            id = btn.data('id');
            var url = '{{ route("partner.delete", ":id") }}';
